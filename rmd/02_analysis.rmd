...
```{r prep}
set.seed(1)
required_packages <- c("tidyverse", "magrittr", "DBI", "bigrquery", "arrow","glue", "vroom","janitor", "gt", "ggwordcloud", "readxl", "ggthemes", "hrbrthemes", "extrafont", "plotly", "scales", "stringr", "gganimate", "here", "tidytext", "sentimentr", "scales", "DT", "here", "sm", "mblm", "glue", "fs", "knitr", "rmdformats", "janitor", "urltools", "colorspace", "pdftools", "showtext", "pander", "wordcloud2")
...
questions %>% mutate(prop = volume / total_volume) %>% 
  ...
  scale_y_continuous(labels = scales::percent, limits = c(0, 0.006), expand = c(0,0)) +
...
volume_top %>% 
  ...
  labs(x = "", title = "Volume of searches", fill = "") + 
  annotate("text", x = 7800, y = 75*10^6, label = "Remaining 99.9967%") + 
  geom_segment(aes(x = 7900, y = 35*10^6, xend = 10000, yend = 35*10^6),
...
```{r search_category_volume_mean, fig.height = 6, fig.width = 7}
df %>% 
...
```{r search_category_total_volume, fig.height = 6, fig.width = 7}
df %>% 
  mutate(volume = volume / sum(volume)) %>% 
  ...
  theme_light() +
  coord_flip() +
  scale_y_continuous(limits = c(0, 0.25), expand = c(0,0), labels = scales::percent) +
  labs(x = "", y = "", title = "Total search volume by category")
```
...
```{r search_category_cpc_mean, fig.height = 6, fig.width = 7}
df %>% 
  ...
  labs(x = "", y = "", title = "Mean CPC by category")
```
