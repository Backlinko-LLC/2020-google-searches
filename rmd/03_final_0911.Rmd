---
title: "We Analyzed 306M Keywords. Here's What We Learned About Google Searches in 2020"
author: 
...
!!!D: Please modify based on your recent analysis. Please refer out to the annex to know more about our approach. 

```{r top, echo=FALSE}
title_description_table <- read.csv(here::here("plots", "csv", "misspell.csv"), stringsAsFactors=F) %>% 
    select(Search = spell, Proportion = volume_perc) %>% 
    head(50)
...
!!!D: Can you add a bit more meat to the bones here wehn describing the graph. e.g. how many keywords are each category, what are their mean values, etc. Just a few examples. 

```{r search_tails, echo=FALSE}
knitr::include_graphics(here::here("plots", "0911", "volume_by_searches.png"))
```
...
knitr::include_graphics(here::here("plots", "0911", "volume_percentage.png"))
```
...
knitr::include_graphics(here::here("plots", "0911", "questions_count.png"))
```

Although only about 1% of the total volume of searches are questions. 

!!!D: please adjust description with the new seach volume if necessary. 

```{r questions_volume, echo=FALSE}
knitr::include_graphics(here::here("plots", "0911", "questions_volume.png"))
```
...
!!!D: please add a sentence on mean/median of keyword length. Also, add a sentence on something like "x% of total search volume are with keywords between 2 and 20 characters long." He wants to have a bit more insights on the graphs
...
knitr::include_graphics(here::here("plots", "0911", "volume_keywordlength.png"))
```
...
!!!D: please add a sentence on mean/median of number of words. Also, add a sentence on something like "x% of total search volume are with keywords between 1 and 5 words long long." 
...
!!!D: Please write here the mean/median keyword difficulty. 
...
!!!D: What are the means/median number of SERP features for the different keyword difficultie categories? e.g. Easy keyword difficulty category (0-10) has around x Serp features on average. 
...
!!!D: Please make sure the sentences have proper punctuation, grammer, etc.
...
!!!D: Please provide a rough plot with the timeline (last 12 month) with the highest spikes. You mentioned during our call it wasn´t very insightful, but let´s add that here to have at least something if you have the code at hand.  


# Annex

!!!D: I would add here the top 50 analysis with ahrefs for the adjusted search volume. May be a bit shortened to explain your approach. 
