...
write_questions_volume <- function(){
  ...
  write_csv(questions, "../proc_data/questions_volume_nospell.csv")
}
#write_questions_volume()
questions <- read_csv("../proc_data/questions_volume_nospell.csv")

questions %>% mutate(prop = volume / overview$volume) %>% 
  ggplot(aes(x = reorder(question, desc(prop)), y = prop)) +
...
questions %>% mutate(prop = n / overview$count) %>% 
  ggplot(aes(x = reorder(question, desc(prop)), y = prop)) +
  geom_bar(stat = "identity", fill = bl_dark, color = "black") +
  geom_text(aes(label = glue::glue("{format(round(prop, 4) * 100, scientific = FALSE)}%")),
            nudge_y = .0025, family = "Montserrat", fontface = "bold", 
            color = "grey40") +
  scale_x_discrete(expand = c(.12, .12)) +
  scale_y_continuous(expand = c(0, 0),
                     limits = c(0, .09)) +
  labs(title = "“How” (8.07%), “What” (3.4%), and “Where” (.88%) Keywords\nAre the Most Common Types of Question Keywords", x = NULL, y = NULL) +
  theme_blank + 
  theme(plot.title = element_text(margin = margin(b = 7))) +
  ggsave(here::here("plots", "reworked_font", "questions_count.pdf"),
         width = 10, height = 6, device = cairo_pdf)
```
...
tibble("Total percentage of searches that are questions" = 
         scales::percent(questions %$% sum(n) / overview$count, accuracy = 0.001)) %>% 
  pander()
```
