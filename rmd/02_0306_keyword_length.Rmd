...
kw_length %>% filter(length <= 50) %>% 
    ...
    labs(x = "Search term length (in characters)", y = "Proportion of total search volume", title = "Search volume by length of search term") +
    theme(panel.grid.minor.x = element_blank())
```
...
```{r keyword_length_count, fig.width = 10, fig.height = 6}
kw_length %>% filter(length <= 50) %>% 
