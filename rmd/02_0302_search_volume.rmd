...
n_orig_prop %>% 
  ...
  ggsave(here::here("plots", "reworked", "proportion_search_volume.pdf"),
...
n_orig_prop %>% 
  ...
  scale_y_continuous(labels = scales::percent, limits = c(0, 1), expand = c(0.001, 0.001))+
  ggsave(here::here("plots", "reworked", "proportion_searches.pdf"),
         width = 10, height = 7.5, device = cairo_pdf)
```
...
volume_top %>% 
  ...
  labs(x = "Rank (top Keywords)", title = "Volume of top searches", fill = "") + 
  ...
  ggsave(here::here("plots", "reworked", "volume_by_searches_dataforseo.pdf"),
...
top100 %>% add_rownames() %>% 
    ...
    ggsave(here::here("plots", "reworked", "volume_by_searches_ahref.pdf"),
           width = 10, height = 6, device = cairo_pdf)
```